...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			var title = '';
			if(_this.find("i.busfont,i.layui-icon").attr("data-icon") != undefined){
				if(_this.find("i.busfont").attr("data-icon") != undefined){
					title += '<i class="busfont '+_this.find("i.busfont").attr("data-icon")+'"></i>';
				}else{
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				//当前窗口内容
				var curmenu = {
					"icon" : _this.find("i.busfont").attr("data-icon")!=undefined ? _this.find("i.busfont").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
